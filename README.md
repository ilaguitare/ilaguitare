# Hi, I'm Ithar Kazem 👋

Welcome to my GitHub! I'm a passionate engineer and designer with a love for robotics and creative tech projects. You can check out my personal portfolio here:

[![My Website](https://img.shields.io/badge/Visit-My%20Website-blue?style=for-the-badge&logo=googlesites)](https://itharkazem.fr)

I have experience in web development and have contributed to the student community. I was part of the **BDA** at IMTBS and developed their first website: [bda-imtbs-tsp.fr](https://bda-imtbs-tsp.fr). You can also explore the BDA GitHub here: [BDA-TSP-IMTBS](https://github.com/BDA-TSP-IMTBS/).

## Currently Working On

- **SO 101 Arm** – Developing a robotic arm project with embedded control systems. Check out the GitHub here: [SO 101 Arm](https://github.com/)  
- **Controlling a Mini Robot using an EMG Sensor** – Integrating muscle signal sensors to drive a small robot.  
- **Implementing Image Recognition Algorithms on Microcontrollers** – Optimizing image recognition for microcontrollers (not as powerful as a computer).  

## About Me

- Passionate about **robotics** and **embedded systems**  
- Skilled in **web development**, **graphic design**, and **creative tech projects**  
- Always exploring ways to combine technology and art  

Feel free to explore my projects and get in touch!
